...
В личке файлы с общим `media_group_id` собираются в одно обращение: каждый исходный update
сохраняется до ACK, а два секунды тишины после последнего нового файла закрывают сборку при
следующем drain (штатный poll идёт раз в пять секунд). Queue row lock сериализует enqueue и claim;
первая полученная запись владеет lease/binding, остальные связаны `media_group_leader_id` и
завершаются с ней одним SQL statement. Состав закрытой пачки неизменен и переживает recovery.
В Eve передаются общий массив attachments и подписи в порядке update ID; реальное сообщение с
подписью служит reply/source anchor. Импорт принимает до десяти файлов; каждый сохраняет свой
исходный Telegram message ID, каталог и operationKey, поэтому одинаковые имена и повтор одного файла
не сливаются, а lookup изображения по message ID остаётся однозначным. Поздний файл вызывает уведомление
`AGENT_TELEGRAM_MEDIA_GROUP_LATE`, а не второй модельный ход. Групповой журнал сохраняет свой
поштучный контракт источников; сборка относится только к личным чатам. Старые ingress-записи не
перегруппировываются. Реализация: `telegram-media-group*.ts`, миграция 091.
...
`agent/lib/prompt/group-fragments.ts`, одинаковы для семейной и внешней группы и выдаются только живому
корневому ходу: запланированный запуск и дочерний агент сами в чат ничего не доставляют, поэтому не
получают ни маркера, ни `[[split]]`, ни правил реакций. Отбивка перед вызовом инструмента, содержащая
маркер, отбрасывается, как и реакция.
...
Доступные операции напоминаний, агентных расписаний и личных настроек уведомлений исполняются
по явной просьбе без HITL-подтверждения, включая изменение и удаление. Уточнять нужно только
недостающие или неоднозначные данные; во внешней группе вопрос задаётся обычным сообщением.
Несколько напоминаний обрабатываются отдельными вызовами без поштучного согласования.
Текст напоминания доставляется без вызова модели: сочиняемые сообщения готовятся до записи,
а новая работа в момент запуска требует агентного расписания. Проверки доступа, лимиты и
operationKey остаются обязательными на исполнении.
