...
Политика памяти. Отбор: сохраняется только то, что изменит будущий ответ или о чём спросят;
длинное обсуждение это одна запись episode в слоте `итог обсуждения` с `subject.label` = тема, ход
спора не пишется; ноль записей нормальный итог. События (`kind: episode`) несут `occurred_at`;
`search_memories` принимает окно `occurredAfter`/`occurredBefore`. Профильные записи имеют слот
`attribute`; новая запись того же субъекта и слота помечает прежнюю `superseded`
(`memory-slot-supersede.ts`): смысловые виды делят один слот по субъекту и `attribute` независимо от
kind, episode только в слоте итога. Старение без удаления (`memory-retention-score.ts`, SQL в
`memory-retrieval-repository.ts` зеркалит формулу): `R = exp(-age/S)`, age от `last_reinforced_at`,
иначе `occurred_at`, иначе `created_at`; `S = S0 × (1 + ln(1 + n))`, `S0` 30 дней для episode, 60
для итога обсуждения, 180 для profile/preference/fact/family_shared; ранг поиска умножается на
`0.3 + 0.7 × R`, автоблок хода берёт только `R ≥ 0.2`, `search_memories` видит всё. Подкрепление
только за использование: при показанных записях модель заканчивает ответ
`<memory-used>ref,ref</memory-used>` (пустой тег значит «ничего не пригодилось»),
`completedTelegramOutput` вырезает директиву, канал подкрепляет только refs, показанные в этом ходе
(`memory_context_exposures` для текущего `session_turn`, `search_memories` тоже пишет показы),
остальное логируется как `AGENT_MEMORY_REINFORCE_REF_UNKNOWN`; ответ без директивы при показанных
записях пишет `AGENT_MEMORY_USED_MISSING`, ответ из одной директивы `AGENT_MEMORY_USED_DIRECTIVE_ONLY`.
Напоминание `MEMORY_USED_REMINDER` стоит сразу после записей в блоке памяти: правило только в блоке
режима модель игнорировала (1 директива на 59 ответов; слепой эвал на 18 реальных ходах: 1/36,
с обязательным напоминанием 34/36, все refs из показанных). Соседи при записи
(`memory-near-duplicate.ts`): для смыслового вида без `attribute` содержимое эмбеддится и ищутся
активные записи того же субъекта с косинусом `≥ 0.9` (до двух); вставки нет, `remember` возвращает
`AGENT_MEMORY_NEAR_DUPLICATE` с кандидатами, модель повторяет вызов с `reinforces` (то же самое),
`attribute` (факт изменился) или `distinct: true` (другой факт). Порог автоматической склейки
невозможен: эмбеддер даёт 0,92–0,94 и для разных фактов об одном человеке; недоступный эмбеддер
гейт пропускает (`AGENT_MEMORY_NEAR_DUPLICATE_SKIPPED`). Константы в `memory-config.ts`.
Автоподборка и карточка профиля ведут учёт показов (`memory-context-exposure-repository.ts`,
миграция 091): запись, показанная в последние 10 ходов той же application-сессии, в автоподборку не
...
сутки. Тихая проверка памяти идёт по лейнам каждого личного чата и группы минутным диспетчером:
батч из 1–50 непроверенных сообщений материализуется при десяти накопленных, после десяти минут
тишины при хотя бы пяти или после шести часов тишины при любом числе; группа подшивает хвост к
обычному ходу только от восьми сообщений (`memory-review-config.ts`, миграция 089); фоновый батч
получает до 20 уже проверенных сообщений в `<preceding_context>`, видит уже сохранённые записи
разговора в `<existing_memory>`, завершение пишет `AGENT_MEMORY_REVIEW_RESULT`. Миграция 092
лечит лейны, вставшие за головой в failed/ambiguous с завершённым наследником.
