...
describeWithDatabase("telegramIngressRepository", () => {
  it("lets concurrent drainers claim different queues but never two heads of the same queue", async () => {
    ...
    const claims = (await Promise.all(Array.from({ length: 6 }, () =>
      telegramIngressRepository.claimNext(LEASE_MILLISECONDS, NO_BURST_WAIT)))).filter((claim) => claim !== null);
    expect(claims.map((claim) => claim.updateId).sort()).toEqual(["1001", "1003"]);
    expect(await telegramIngressRepository.claimNext(LEASE_MILLISECONDS, NO_BURST_WAIT)).toBeNull();
    ...
    expect((await telegramIngressRepository.claimNext(LEASE_MILLISECONDS, NO_BURST_WAIT))?.updateId).toBe("1002");
  });
  beforeEach(async () => {
  ...
  it("keeps a rejected update tombstoned after the group becomes family-private", async () => {
    ...
    await expect(telegramIngressRepository.claimNext(LEASE_MILLISECONDS, NO_BURST_WAIT)).resolves.toBeNull();
  });
  ...
  it("claims only the oldest update per queue while another queue progresses", async () => {
    ...
    const first = await telegramIngressRepository.claimNext(LEASE_MILLISECONDS, NO_BURST_WAIT);
    const independent = await telegramIngressRepository.claimNext(LEASE_MILLISECONDS, NO_BURST_WAIT);
    const blocked = await telegramIngressRepository.claimNext(LEASE_MILLISECONDS, NO_BURST_WAIT);
    ...
    await expect(telegramIngressRepository.claimNext(LEASE_MILLISECONDS, NO_BURST_WAIT)).resolves.toMatchObject({
      updateId: "2002",
    });
  });
  ...
  it("persists a monotonic Eve event cursor atomically with ingress completion", async () => {
    ...
    const second = await telegramIngressRepository.claimNext(LEASE_MILLISECONDS, NO_BURST_WAIT);
  ...
  it("reclaims an expired lease and rejects the stale worker token", async () => {
    ...
    const reclaimed = await telegramIngressRepository.claimNext(LEASE_MILLISECONDS, NO_BURST_WAIT);
