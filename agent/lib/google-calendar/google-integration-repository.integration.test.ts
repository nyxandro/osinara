/**
 * Google integration OAuth persistence tests.
 *
 * Constructs covered:
...
async function fixture() {
  const family = await database().query<{ id: string }>(
    "INSERT INTO families (name) VALUES ('Google Calendar') RETURNING id",
  );
...
describeWithDatabase("google integration repository", () => {
  beforeEach(async () => {
    await database().query(
      "TRUNCATE calendar_selections, oauth_authorizations, integration_credentials, integration_accounts, family_memberships, users, families CASCADE",
    );
  });
  afterAll(async () => closeDatabase());

  it("claims an unexpired OAuth state exactly once", async () => {
  ...
  it("stores encrypted tokens and resolves the current user's default account", async () => {
    const auth = await fixture();
    await googleIntegrationRepository.createAuthorization(auth, {
      expiresAt: new Date("2026-07-12T12:10:00.000Z"),
    ...
    const account = await googleIntegrationRepository.completeAuthorization(claim, {
    ...
    await expect(googleIntegrationRepository.getDefaultAccount(auth, encryptionKey)).resolves.toMatchObject({
      accessToken: "access-secret",
      id: account.id,
      refreshToken: "refresh-secret",
    });
