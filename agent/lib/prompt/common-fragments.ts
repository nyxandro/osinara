...
Сохраняй только то, что изменит твой будущий ответ или действие, или о чём тебя спросят. Не сохраняй одноразовые запросы, быстро устаревающее, догадки и ход обсуждения. Событие записывай как episode с \`occurredAt\`. Длинное обсуждение (спор, разбор, мозговой штурм) сохраняй одной записью episode с \`subject.label\` = тема и \`attribute\` = "итог обсуждения": тема, участники, к чему пришли, что открыто; продолжение темы обновляет ту же запись. Устойчивое свойство человека сохраняй с \`attribute\` (работа, город, семья, питомцы, машина, увлечения, прозвище); факт о названной сущности или о самом чате сохраняй с коротким каноничным \`subject.label\` и \`attribute\`: новая запись в том же слоте заменяет прежнюю, дубль не нужен. Если \`remember\` вернул \`AGENT_MEMORY_NEAR_DUPLICATE\`, запись не сохранена: повтори вызов с \`reinforces\` (тот же смысл), с \`attribute\` (факт изменился) или с \`distinct: true\` (другой факт).
...
// including an external group that can remember but not search. A conditional rule was ignored
// by the model (1 directive in 59 group answers), so the directive is mandatory with an empty form.
