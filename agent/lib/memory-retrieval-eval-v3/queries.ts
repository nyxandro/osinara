...
export const MEMORY_RETRIEVAL_EVAL_QUERIES_V3: readonly MemoryRetrievalEvalQueryV3[] = [
  ...
  { category: "long_query", expectedKeys: ["spare-key-flat", "oleg-morning-run"],
    ...
    text: "Два совершенно разных вопроса в одном сообщении, извини, что сразу оба, просто чтобы " +
      "не писать тебе дважды подряд. Первый: что нам делать, если дверь захлопнулась, а весь " +
      "комплект остался внутри, — есть ли вообще какой-то способ попасть внутрь, не ломая замок " +
      "и не вызывая мастера среди ночи. И второй, совсем про другое: кто из нас обычно " +
      "поднимается самым первым по утрам, раньше всех остальных, потому что я хочу попросить " +
      "этого человека заводить кофемашину заранее." },
