...
export const MEMORY_RETRIEVAL_EVAL_QUERIES_V3: readonly MemoryRetrievalEvalQueryV3[] = [
  ...
  // Длинный запрос: больше девятисот символов, то есть несколько кусков эмбеддинга. Длина
  // привязана к `MEMORY_EMBEDDING_CHUNK_MAX_CHARACTERS`: когда предел поднимали с 400 до 900,
  // прежние четырёхсотсимвольные запросы стали укладываться в один кусок, и категория молча
  // перестала проверять то, ради чего заведена. Тест держит это числом кусков, а не длиной.
  { category: "long_query", expectedKeys: ["spare-key-dacha", "dacha-boiler", "lawnmower-fuel"],
    key: "long-dacha-trip",
    text:
      "Собираемся в субботу на дачу, выезжаем рано утром, и я хочу заранее всё вспомнить, " +
      ...
      "надо не забыть про газонокосилку и про то, чем мы её в прошлый раз заправляли. Вообще " +
      "хочу в этот раз собраться нормально, а не как обычно: сумки складываем накануне вечером," +
      " а не утром в спешке, и проверяем всё по списку, чтобы не разворачиваться с полпути. " +
      "Дети мёрзнут, если дом холодный, поэтому важно попасть туда не поздно и сразу всё " +
      "запустить, а не сидеть в куртках до ночи. Обед, наверное, возьмём с собой, чтобы не " +
      "искать ничего по дороге и не терять время на остановки. Если на месте окажется, что " +
      "чего-то не хватает, в ближайший магазин ехать далеко, и это опять выбьет нас из " +
      "расписания на полдня, а хочется всё-таки успеть заняться участком, пока светло, и " +
      "вернуться не слишком поздно."},
  { category: "long_query",
    expectedKeys: ["alena-school", "pool-subscription", "alena-swimming", "alena-gymnastics"],
    key: "long-school-year",
    text:
      "Скоро учебный год, и я пытаюсь собрать в голове всё, что связано с младшей: в какой она " +
      "класс переходит и в какой школе это вообще происходит, потому что я всё время путаю " +
      ...
      "которые у неё стоят в середине недели, и я про них тоже всё время забываю. Хочу " +
      "разложить всю её неделю по дням и посмотреть, где остаётся свободное время, потому что в" +
      " прошлом году получилось так, что она приходила домой поздно и делала уроки уже " +
      "уставшая. Если окажется, что два занятия стоят подряд, надо будет что-то переносить, и " +
      "лучше договориться об этом сейчас, а не в середине сентября, когда все расписания уже " +
      "утвердят. Заодно проверю, что мы ничего не пропустили с оплатой, потому что напоминания " +
      "приходят на почту, которую я открываю раз в месяц, и один раз мы так уже пропустили срок" +
      " и потом доплачивали отдельно, чего совсем не хочется повторять в этом году."},
  { category: "long_query", expectedKeys: ["ladoga-deploy", "ladoga-freeze", "merge-policy"],
    key: "long-release-window",
    text:
      "Хочу спланировать выкатку на следующую неделю и заранее понять ограничения, чтобы не " +
      "устраивать спешку в последний момент. Напомни, в какой день недели мы вообще " +
      "разворачиваем проект и на каком стенде это происходит, и какое у нас было правило про " +
      "вечер пятницы, потому что я помню, что мы о чём-то договаривались, но детали вылетели. И" +
      " ещё, что именно должно быть зелёным, прежде чем сливать в основную ветку. Хочется " +
      "собрать всё это в один список и повесить перед глазами, потому что каждый раз мы " +
      "вспоминаем правила уже в процессе и спорим о них на ходу. Если окно выкатки жёстко " +
      "привязано к дню недели, то планировать надо от него назад, а не от готовности кода, и " +
      "тогда понятно, к какому моменту всё должно быть готово. Отдельно хочу понимать, кто " +
      "принимает решение, если что-то пошло не так уже после выкатки, и в какой момент мы " +
      "считаем, что откатываемся, а не чиним на ходу. В прошлый раз мы этот момент упустили и " +
      "потратили вечер на починку того, что проще было откатить за пять минут и разобрать " +
      "спокойно на следующий день."},
  { category: "long_query", expectedKeys: ["fedor-pressure-medicine", "fedor-followup"],
    key: "long-grandfather-health",
    text:
      "Завтра еду к отцу и хочу заранее всё уточнить, потому что каждый раз что-нибудь упускаю " +
      "и потом перезваниваю. Он после больницы принимает лекарство, и я не помню, сколько раз в" +
      " день его нужно пить и в какое время, утром или вечером, или и так и так. А ещё ему " +
      "назначали что-то измерять регулярно после выписки, и я хочу проверить, делает ли он это " +
      "вообще, как часто это нужно делать и записывает ли он показания. Он человек аккуратный, " +
      "но живёт один, и половину назначений держит в голове, а не на бумаге, поэтому я хочу " +
      "приехать и убедиться сам, а не выяснять это по телефону между делом. Если окажется, что " +
      "он что-то пропускает, лучше это заметить сейчас, а не через месяц на приёме. Заодно " +
      "посмотрю, хватает ли ему запаса до следующего визита, потому что аптека рядом с ним " +
      "маленькая и нужного там бывает не каждый день, и если придётся искать по другим, то " +
      "лучше заняться этим заранее, пока я на машине и никуда не тороплюсь, а не в последний " +
      "момент, когда пачка уже закончится и он будет пропускать приём."},
  { category: "long_query", expectedKeys: ["petr-bicycle-request", "petr-bicycle"],
    key: "long-bicycle",
    text:
      "Старший давно просит велосипед получше, и я хочу вернуться к этому разговору, но сначала" +
      " вспомнить, о чём мы вообще договорились и на какой месяц отложили решение. Заодно нужно" +
      " понять, что у него сейчас за велосипед: какого он цвета, есть ли на нём багажник и " +
      "какой у него номер рамы, потому что если продавать, то это спросят в первую очередь, а " +
      "искать документы по всей квартире совершенно не хочется. Хочу подойти к разговору " +
      "подготовленным, чтобы не получилось как в прошлый раз, когда я не помнил собственных " +
      "слов и мы начали всё заново. Если мы называли конкретный срок, то честнее его и " +
      "держаться, а если срок был привязан к чему-то другому, надо понять, наступило это или " +
      "нет. Сам велосипед, наверное, будем продавать, а не отдавать, и тогда описание надо " +
      "составить заранее, со всеми приметами, чтобы не переписываться с покупателями по каждой " +
      "мелочи. И ещё хочу понять, сколько мы вообще готовы добавить сверху, потому что без этой" +
      " цифры разговор опять превратится в обсуждение моделей вместо решения."},

  // Длинное сообщение, обе темы которого доступны только смысловой ветке: общих слов с нужными
  // записями почти нет. Оно нарочно разрезается на два куска эмбеддинга, по одному на тему —
  // именно здесь усреднение кусков в один вектор даёт точку, не похожую ни на одну из тем.
  { category: "long_query", expectedKeys: ["spare-key-flat", "oleg-morning-run"],
    key: "long-two-paraphrases",
    text:
      "Два совершенно разных вопроса в одном сообщении, извини, что сразу оба, просто чтобы не " +
      "писать тебе дважды подряд. Первый: что нам делать, если дверь захлопнулась, а весь " +
      "комплект остался внутри, — есть ли вообще какой-то способ попасть внутрь, не ломая замок" +
      " и не вызывая мастера среди ночи. Спрашиваю не потому, что это уже случилось, а потому " +
      "что однажды почти случилось, и мы тогда поняли, что никакого плана у нас нет, и на этом " +
      "разговор закончился. Хочу, чтобы в следующий раз ответ был известен заранее, а не " +
      "придумывался на лестничной площадке в час ночи. И второй, совсем про другое: кто из нас " +
      "обычно поднимается самым первым по утрам, раньше всех остальных, потому что я хочу " +
      "попросить этого человека заводить кофемашину заранее. Она долго выходит на режим, и если" +
      " включать её в момент, когда уже собираешься уходить, то ждать приходится как раз те " +
      "минуты, которых утром не хватает. Просить кого попало неудобно, поэтому и спрашиваю, кто" +
      " встаёт раньше всех и кому это вообще будет по дороге."},
