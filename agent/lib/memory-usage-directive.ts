 ...
 * - `readMemoryUsageDirective`: the answer without the line, plus the refs it named.
 *
...
// Whole line only, at column zero: indented, it belongs to a Markdown code block and stays there.
const DIRECTIVE_LINE_PATTERN = new RegExp(
  `^\\[${DIRECTIVE_NAME}:[ \\t]*(?<body>[^\\]\\n]*)\\][ \\t\\r]*$`,
  "imu",
);
...
export const MEMORY_USAGE_INSTRUCTION = [
  `Последней строкой ответа всегда пиши служебную строку [${DIRECTIVE_NAME}: ...] и перечисляй в ней memoryRef тех записей выше, на которых держится ответ.`,
  `Если ни одна не пригодилась, напиши [${DIRECTIVE_NAME}: нет] — это нормальный ответ, а не ошибка.`,
  "Строка служебная: человек её не увидит, транспорт вырезает её до отправки. Ссылаться можно только на записи из этого блока.",
].join(" ");

export interface MemoryUsageDirective {
...
export function readMemoryUsageDirective(text: string): MemoryUsageDirective {
  const match = DIRECTIVE_LINE_PATTERN.exec(text);
  if (match === null) return { answer: text, declared: false, memoryRefs: [] };
  const refs = [...(match.groups?.body ?? "").matchAll(REF_PATTERN)].map((ref) => ref[0]);
  return {
    // The line stands alone, so the newline that carried it goes with it.
    answer: text.replace(DIRECTIVE_LINE_PATTERN, "").replace(/\n{3,}/gu, "\n\n").trim(),
    declared: true,
