 ...
 *
 * Constructs covered:
 * - Generation-zero compatibility and route re-keying.
...
describeWithDatabase("session repository", () => {
  ...
  it("continues generation zero and follows Telegram anchor re-keying", async () => {
    ...
    await expect(sessionRepository.resolveContinuationToken("101:42:900"))
      .resolves.toBe("101:42:900");
  });
  ...
  it("defers a requested rotation until the pending operation completes", async () => {
    ...
    await sessionRepository.recordTurnCompleted(current.id, "wrun_old");
  ...
  it("accepts a newer Eve root and ignores delayed events from the replaced root", async () => {
    ...
    await expect(sessionRepository.recordTurnCompleted(current.id, oldRoot)).resolves.toBe("stale");
    ...
    const stored = await database().query<{
      ...
      "SELECT completed_turns, eve_session_id FROM conversation_sessions WHERE id = $1",
      [current.id],
    );
    expect(stored.rows[0]).toEqual({ completed_turns: 0, eve_session_id: newRoot });
  });
