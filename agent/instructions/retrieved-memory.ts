 ...
 * The retrieved payload is then lifted into the turn tail at the transport boundary by
 * `lib/prompt/turn-memory-projection.ts`; service notices stay in the instruction prefix.
 */
...
export default defineDynamic({
  events: {
    "turn.started": async (event, ctx) => {
      ...
      return defineInstructions({ content: block, role: "system" });
    },
  },
});
